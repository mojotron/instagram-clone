export const MAX_IMAGE_SIZE = 2000000;
export const MAX_AVATAR_IMAGE_SIZE = 500000;
export const MAX_UPLOAD_LIMIT = 3;
export const MAX_COMMENTS_LIMIT = 5;
export const MAX_POST_COMMENTS_ALERT_MSG = `
Maximum number of comments is limited to 5,
and maximum number of replies on comment is 3.
This project is for learning purposes!
Thank you for inspecting my project!
`;
export const MAX_POST_PER_USER = 9;
export const MAX_POST_PER_USER_MSG = `
Maximum number of posts per user is limited to 9.
To minimize firebase memory use, this project is for learning
purpose and. Thank you! 
`;
export const TIMELINE_POST_LIMIT = 5;
export const MAX_MESSAGES_LIMIT = 9;
export const MAX_MESSAGES_LIMIT_MESSAGE = `
Maximum number of messages between two user is ${MAX_MESSAGES_LIMIT}.
To minimize firebase memory use, this project is for learning
purpose and. Thank you!
`;
export const MAX_NOTIFICATIONS_NUMBER = 19;
export const TEST_ACCOUNT = {
  email: 'test-account@example.com',
  auth: 'TestAcc0unt?',
};
export const EXPLORE_POST_LIMIT = 9;
